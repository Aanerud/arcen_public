...
#[cfg(test)]
use arcen_identity::{
    validate_direct_resume_grant, DirectResumeValidationContext, LogindSessionId,
};
...
/// scrubbed when the Pier-owned registry is dropped.
...
impl TopologyBinding {
    pub(crate) fn from_response(response: &AuthResponse) -> Result<Self, ResumeRegistryError> {
        ...
        struct Topology<'a> {
            ...
            // Carrier A: when present, binds the exact canonical, already-
            // validated multi-monitor-v1 auth request — the full requested
            // topology roster (including every monitor's rotation and
            // safe-area policy) plus the client's ordered carrier list —
            // into the resume topology hash too, not only the legacy
            // primary-compatible `monitors` roster above. A reconnect that
            // silently changed any rotation, safe-area policy, or carrier
            // preference must bind as a genuinely different topology,
            // exactly like a changed screen size or monitor count already
            // does — never silently accepted as the "same" committed
            // topology across a resume. `AuthMultiMonitorRequestMsg`
            // derives `Serialize` over plain, already-validated fields, so
            // this hash is stable and deterministic for identical input.
            // Absent (`None`) for a legacy/non-multi-monitor client, in
            // which case `skip_serializing_if` omits the field entirely,
            // producing byte-identical serialized bytes (and thus the same
            // hash) as before this field existed — legacy resume binding
            // behavior is completely unchanged.
            #[serde(skip_serializing_if = "Option::is_none")]
            multi_monitor_v1: &'a Option<arcen_protocol::messages::AuthMultiMonitorRequestMsg>,
        }
        ...
        let bytes = serde_json::to_vec(&Topology {
            ...
            multi_monitor_v1: &response.multi_monitor_v1,
        })
...
impl ResumeRegistry {
    ...
    pub(crate) fn revoke(
        ...
        self.begin_drain(active_session_id)?;
        self.complete_drain(active_session_id)
    }
    ...
    pub(crate) fn shutdown(&self) -> Result<(), ResumeRegistryError> {
        let owner = self.lock_entry()?.as_mut().map(|entry| {
            entry.slot.revoke();
            entry.state = EntryState::Draining;
            entry.owner.clone()
        });
        if let Some(owner) = owner {
            let _ = owner.send(OwnerCommand::PierShutdown);
        }
        Ok(())
    }

    pub(crate) fn complete_shutdown(&self) -> Result<(), ResumeRegistryError> {
        let mut entry = self.lock_entry()?;
        if entry
            .as_ref()
            .is_some_and(|entry| entry.state == EntryState::Draining)
        {
            *entry = None;
        }
        Ok(())
...
mod tests {
    ...
    #[test]
    fn token_validation_binds_host_uid_logind_session_holder_disclaimer_and_time() {
        ...
        *last = if *last == b'0' { b'1' } else { b'0' };
        ...
        changed_principal.native_principal = NativePrincipal::Linux {
            uid: 1002,
            logind_session_id: LogindSessionId::new("c7").unwrap(),
        };
        assert!(matches!(
        ...
        changed_principal.native_principal = NativePrincipal::Linux {
            uid: 1001,
            logind_session_id: LogindSessionId::new("c8").unwrap(),
        };
        assert!(matches!(
            validate(&changed_principal, expected.holder_nonce, 1_000),
            Err(DirectResumeValidationError::Claims(
        ...
        changed_session.active_session_id = ActiveHostSessionId::new("linux-logind:c8").unwrap();
        assert!(matches!(
    ...
    #[test]
    fn registry_rejects_the_exact_expiry_without_rotating() {
        let clock = FakeClock::new(2_000);
        let registry = ResumeRegistry::with_clock(clock.clone()).unwrap();
        let mut response = AuthResponse::resume("03".repeat(32), "placeholder");
        let mut expected = bindings("host-a");
    ...
    // Carrier A: helper builders for the tests below, mirroring
    // `session::multi_monitor`'s own test fixtures so the canonical validated
    // `AuthMultiMonitorRequestMsg` payload can be exercised here too, without
    // depending on (or duplicating fields across) that module's private test
    // helpers.
    fn multi_monitor_descriptor(
        id: &str,
        rotation: arcen_protocol::messages::RotationMsg,
        safe_area_policy: arcen_protocol::messages::SafeAreaPolicyMsg,
    ) -> arcen_protocol::messages::RequestedMonitorDescriptorMsg {
        arcen_protocol::messages::RequestedMonitorDescriptorMsg {
    ...
    fn multi_monitor_request(
        rotation: arcen_protocol::messages::RotationMsg,
        safe_area_policy: arcen_protocol::messages::SafeAreaPolicyMsg,
        carriers: Vec<arcen_protocol::messages::MultiMonitorCarrierMsg>,
    ) -> arcen_protocol::messages::AuthMultiMonitorRequestMsg {
        arcen_protocol::messages::AuthMultiMonitorRequestMsg::new(
            arcen_protocol::messages::RequestedMonitorTopologyMsg::new(vec![
                multi_monitor_descriptor("display-1", rotation, safe_area_policy),
            ])
            .expect("requested topology"),
            carriers,
        )
        .expect("auth multi-monitor request")
    }

    #[test]
    fn topology_binding_is_unaffected_by_absent_multi_monitor_v1_legacy_request() {
        // A legacy/non-multi-monitor client never sets `multi_monitor_v1`, so
        // the resume topology binding computed after this field was added
        // must still exactly match the binding a plain single-monitor client
        // (with no multi-monitor request at all) would have produced before
        // it existed — `skip_serializing_if` must actually omit the field
        // rather than serialize it as an explicit `null`.
        let mut response = AuthResponse::pam("user", "password");
        response.screen_width = 1920;
        response.screen_height = 1080;
        response.displays_mode = "single_primary".to_string();
        assert!(response.multi_monitor_v1.is_none());

        let with_none_explicitly_reasserted = TopologyBinding::from_response(&response).unwrap();
        // Reassigning `None` to `None` must be a total no-op on the computed
        // binding; this also documents that the field genuinely participates
        // in (or, when absent, is dropped from) the serialized payload
        // rather than being ignored by a stray unused-field bug.
        response.multi_monitor_v1 = None;
        assert_eq!(
            with_none_explicitly_reasserted,
            TopologyBinding::from_response(&response).unwrap()
        );
    }

    #[test]
    fn topology_binding_roundtrips_identically_for_an_identical_multi_monitor_v1_payload() {
        let mut response = AuthResponse::pam("user", "password");
        response.screen_width = 1920;
        response.screen_height = 1080;
        response.multi_monitor_v1 = Some(multi_monitor_request(
            arcen_protocol::messages::RotationMsg::Degrees0,
            arcen_protocol::messages::SafeAreaPolicyMsg::StandardFullscreen,
            vec![arcen_protocol::messages::MultiMonitorCarrierMsg::MuxedReliableStream],
        ));

        let first = TopologyBinding::from_response(&response).unwrap();
        let second = TopologyBinding::from_response(&response).unwrap();
        assert_eq!(first, second);

        // An independently rebuilt, field-for-field identical request must
        // bind exactly the same too, proving the hash depends only on the
        // canonical payload's content, not on incidental allocation identity.
        response.multi_monitor_v1 = Some(multi_monitor_request(
            arcen_protocol::messages::RotationMsg::Degrees0,
            arcen_protocol::messages::SafeAreaPolicyMsg::StandardFullscreen,
            vec![arcen_protocol::messages::MultiMonitorCarrierMsg::MuxedReliableStream],
        ));
        assert_eq!(first, TopologyBinding::from_response(&response).unwrap());
    }

    #[test]
    fn topology_binding_changes_when_a_resume_attempt_silently_changes_rotation() {
        let mut response = AuthResponse::pam("user", "password");
        response.screen_width = 1920;
        response.screen_height = 1080;
        response.multi_monitor_v1 = Some(multi_monitor_request(
        ...
        let original = TopologyBinding::from_response(&response).unwrap();

        response.multi_monitor_v1 = Some(multi_monitor_request(
            arcen_protocol::messages::RotationMsg::Degrees90,
            arcen_protocol::messages::SafeAreaPolicyMsg::StandardFullscreen,
            vec![arcen_protocol::messages::MultiMonitorCarrierMsg::MuxedReliableStream],
        ));
        assert_ne!(original, TopologyBinding::from_response(&response).unwrap());
    }

    #[test]
    fn topology_binding_changes_when_a_resume_attempt_silently_changes_safe_area_policy() {
        let mut response = AuthResponse::pam("user", "password");
        response.screen_width = 1920;
        response.screen_height = 1080;
        response.multi_monitor_v1 = Some(multi_monitor_request(
        ...
        response.multi_monitor_v1 = Some(multi_monitor_request(
            arcen_protocol::messages::RotationMsg::Degrees0,
            arcen_protocol::messages::SafeAreaPolicyMsg::FullFrame,
            vec![arcen_protocol::messages::MultiMonitorCarrierMsg::MuxedReliableStream],
        ));
        assert_ne!(original, TopologyBinding::from_response(&response).unwrap());
    }
    ...
    #[test]
    fn topology_binding_changes_when_a_resume_attempt_silently_changes_the_carrier_list() {
        let mut response = AuthResponse::pam("user", "password");
        response.screen_width = 1920;
        response.screen_height = 1080;
        response.multi_monitor_v1 = Some(multi_monitor_request(
        ...
        response.multi_monitor_v1 = Some(multi_monitor_request(
            ...
            arcen_protocol::messages::SafeAreaPolicyMsg::StandardFullscreen,
            vec![
                arcen_protocol::messages::MultiMonitorCarrierMsg::MuxedReliableStream,
                arcen_protocol::messages::MultiMonitorCarrierMsg::PerMonitorReliableStream,
            ],
        ));
        assert_ne!(original, TopologyBinding::from_response(&response).unwrap());
    }
    ...
    #[test]
    fn topology_binding_changes_between_absent_and_present_multi_monitor_v1() {
        // A client that upgrades from a legacy connect (no multi-monitor
        // request at all) to a multi-monitor-v1 resume attempt (or vice
        // versa) must never be treated as the same committed topology.
        let mut response = AuthResponse::pam("user", "password");
        response.screen_width = 1920;
        response.screen_height = 1080;
        let legacy = TopologyBinding::from_response(&response).unwrap();
        ...
        assert_ne!(legacy, TopologyBinding::from_response(&response).unwrap());
    }
    ...
    fn detach_resume_expiry_and_stale_timer_keep_cleanup_boundaries_exact() {
        ...
        struct Probe {
            capenc_stops: usize,
            audio_stops: usize,
            queue_discards: usize,
            input_resets: usize,
            capenc_starts: usize,
            audio_starts: usize,
            fresh_idrs: usize,
            held_display_and_timezone: bool,
            restores: usize,
            revocations: usize,
            finalized: bool,
        }
        ...
        impl Probe {
            fn apply(&mut self, actions: arcen_session::direct_reconnect::ReconnectActions) {
                if actions.stop_media {
                    self.capenc_stops += 1;
                    self.audio_stops += 1;
                    self.queue_discards += 1;
                }
                self.input_resets += usize::from(actions.reset_input);
                if actions.start_media {
                    self.capenc_starts += 1;
                    self.audio_starts += 1;
                    self.fresh_idrs += 1;
                }
                self.held_display_and_timezone |= actions.hold_restore_leases;
        ...
        assert!(probe.held_display_and_timezone);
        assert_eq!(probe.capenc_stops, 1);
        assert_eq!(probe.audio_stops, 1);
        assert_eq!(probe.queue_discards, 1);
        assert_eq!(probe.input_resets, 1);
        ...
        assert_eq!(probe.capenc_starts, 1);
        assert_eq!(probe.audio_starts, 1);
        assert_eq!(probe.fresh_idrs, 1);
        assert_eq!(probe.restores, 1);
        assert_eq!(probe.revocations, 1);
    }

    #[test]
    fn agent_crash_and_pier_shutdown_each_finalize_once() {
    ...
    #[test]
    fn registry_entry_remains_draining_until_cleanup_completes() {
        let registry = ResumeRegistry::with_clock(FakeClock::new(1_000)).unwrap();
        let expected = bindings("host-a");
        let active_session_id = expected.active_session_id.clone();
        let (owner, _commands) = mpsc::unbounded_channel();
        registry
        ...
        registry.begin_drain(&active_session_id).unwrap();
        assert!(registry.entry.lock().unwrap().is_some());
        assert!(!registry.resume_handshake_available().unwrap());
        registry.complete_drain(&active_session_id).unwrap();
        assert!(registry.entry.lock().unwrap().is_none());
    }
}
