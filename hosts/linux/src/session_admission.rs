...
impl SessionAdmissionRuntime {
    ...
    pub fn complete(&self, lease: SessionAdmissionLease) {
        match self.gate.lock() {
            Ok(mut gate) => {
                if let Err(error) = gate.complete(lease) {
                    tracing::error!(target: crate::logging::target::SESSION, %error, "session admission release failed");
                }
            }
            Err(_) => {
                tracing::error!(target: crate::logging::target::SESSION, "session admission gate lock poisoned during release");
            }
        }
    }
}
